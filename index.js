...
const { validateDesc, validateId, validateStatus } = require('./utils/validation')
...
const runCLICmd = async (cmd, args) => {
    try {
        switch(cmd) {
            case Command.ADD:
                let addedTask = await TaskController.addTask(validateDesc(args[0]))
                console.log(MSG_SUCCESS_ADD_TASK(addedTask))
                break
            case Command.UPDATE:
                let updatedTask = await TaskController.updateTaskDesc(
                    validateId(args[0]), 
                    validateDesc(args.slice(1).join(' '))
                )
                console.log(MSG_SUCCESS_UPDATE_TASK(updatedTask))
                break
            case Command.DELETE:
                let success = await TaskController.deleteTask(validateId(args[0]))
                if(success) console.log(MSG_SUCCESS_DELETE_TASK(args[0]))
                break
            case Command.MARK_IN_PROGRESS:
                let taskInProgress = await TaskController.updateTaskStatus(validateId(args[0]), Status.IN_PROGRESS)
                if(taskInProgress.status === Status.IN_PROGRESS) console.log(MSG_SUCCESS_UPDATE_TASK(taskInProgress))
                break
            case Command.MARK_DONE:
                let taskDone = await TaskController.updateTaskStatus(validateId(args[0]), Status.DONE)
                if(taskDone.status === Status.DONE) console.log(MSG_SUCCESS_UPDATE_TASK(taskDone))
                break
            case Command.LIST:
                let tasks = await TaskController.getTaskList(validateStatus(args[0], false))
